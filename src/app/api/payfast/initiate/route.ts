...
export async function POST(request: Request) {
    try {
        ...
        const { isPayFastConfigured, getEnv } = require('@/lib/env');
        if (!isPayFastConfigured()) {
            return NextResponse.json({ 
                success: false, 
                message: 'PayFast integration is not configured. Please use a different payment method.' 
            }, { status: 503 });
        }

        const merchantId = getEnv('PAYFAST_MERCHANT_ID', '');
        const merchantKey = getEnv('PAYFAST_MERCHANT_KEY', '');
        const passphrase = getEnv('PAYFAST_PASSPHRASE', '');
        
        // Base URLs should be configured in env, with fallbacks
        const returnUrl = getEnv('PAYFAST_RETURN_URL', `${process.env.NEXT_PUBLIC_BASE_URL || ''}/checkout/success`);
        const cancelUrl = getEnv('PAYFAST_CANCEL_URL', `${process.env.NEXT_PUBLIC_BASE_URL || ''}/checkout/cancel`);
        const notifyUrl = getEnv('PAYFAST_NOTIFY_URL', `${process.env.NEXT_PUBLIC_BASE_URL || ''}/api/payfast/notify`);
